...
/**
 * NEW (ROBUST): lock ID fields even if casing/spacing differs.
 * This will lock these columns ANYWHERE (pk or fk):
 * - User ID
 * - Vehicle ID
 * - Sale ID
 */
...
function rowHtml(row, cols, pkName) {
  ...
  cols.forEach((col) => {
    ...
    if (isNew || isEditing) {
      // IMPORTANT: lock these ID fields even during edit/create
...
saveBtn.onclick = async () => {
  try {
    ...
    inputs.forEach((inp) => {
      ...
      // Never send these IDs — they’re auto-generated by MySQL
