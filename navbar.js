(function () {
  ...
  function injectNav() {
    ...
    style.textContent = `
      ...
      .nav-brand {
        font-weight: 700; font-size: 20px; color: var(--ink, #0d1117);
        text-decoration: none; display: flex; align-items: center;
        gap: 8px; margin-right: 20px;
      }
      .nav-links { display: flex; gap: 24px; align-items: center; flex: 1; }
      .nav-link {
        color: var(--ink2, #4b5563); text-decoration: none;
        font-weight: 500; font-size: 15px; transition: color 0.15s;
      }
      .nav-link:hover, .nav-link.active { color: var(--accent, #4f46e5); }
      ...
      .nav-btn-primary  { background: var(--accent, #4f46e5); color: white; }
      .nav-btn-primary:hover { filter: brightness(0.95); }
      .nav-btn-secondary { background: var(--surface, white); color: var(--ink, #0d1117); border-color: var(--border, #ccc); }
      .nav-btn-secondary:hover { background: var(--surface2, #f4f6f9); }
      .nav-dm-toggle {
        background: none; border: none; font-size: 18px; cursor: pointer;
        padding: 6px; border-radius: 50%; display: flex; align-items: center;
        justify-content: center; transition: background 0.2s;
      }
      .nav-dm-toggle:hover { background: var(--surface2, #f4f6f9); }
    ...
    // ── Session detection (supports both key formats) ─────────────────────────
    const isPreview  = new URLSearchParams(window.location.search).get("preview") === "1";
    const SESSION_MS = 12 * 60 * 60 * 1000;

    // Format 1: set by app.js  → userProfile + lastLoginAt
    // Format 2: set by cars.html session check → userEmail + loggedInAt
    const userEmail  = localStorage.getItem("userEmail")
                    || (() => { try { return JSON.parse(localStorage.getItem("userProfile") || "null")?.email || ""; } catch { return ""; } })();
    const lastLogin  = Number(localStorage.getItem("lastLoginAt") || localStorage.getItem("loggedInAt") || 0);
    const sessionFresh = lastLogin && (Date.now() - lastLogin < SESSION_MS);
    const isAdmin    = localStorage.getItem("isAdmin") === "1" || localStorage.getItem("isAdmin") === "true";

    const isLoggedIn = isPreview || (!!userEmail && sessionFresh);
    ...
    if (logoutBtn) {
      logoutBtn.addEventListener("click", () => {
        ["userEmail", "loggedInAt", "lastLoginAt", "isAdmin", "userProfile", "userData", "loggedInAt"].forEach(k => localStorage.removeItem(k));
    ...
    if (typeof window.toggleDarkMode === "function") {
      nav.querySelector(".nav-dm-toggle")?.addEventListener("click", window.toggleDarkMode);
    }
  }
